		...
		if (!connection.isConnected()) {
			if (read == ReadPreference.PRIMARY
				...
				self.serverConfig._commandsStore.read_from_writer(
					{   type: 'query', db_command: db_command, options: options, callback: callback, db: self, executeQueryCommand: __executeQueryCommand, executeInsertCommand: __executeInsertCommand
					}
				);
			} else {
...
var __executeInsertCommand = function (self, db_command, options, callback) {
	...
	if (!connection.isConnected()) {
		return self.serverConfig._commandsStore.write(
			{   type: 'insert', 'db_command': db_command, 'options': options, 'callback': callback, db: self, executeQueryCommand: __executeQueryCommand, executeInsertCommand: __executeInsertCommand
			}
		);
	}
