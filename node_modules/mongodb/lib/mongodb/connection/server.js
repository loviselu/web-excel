...
Server.prototype.connect = function (dbInstance, options, callback) {
	...
	connectionPool.on("close", function () {
		...
		if (server.isAutoReconnect()
			&& (server._serverState != 'destroyed' && server._serverState == 'disconnected')) {
	...
	var __attemptReconnect = function (server) {
		return function () {
			...
			server.connect(server.db, server.options, function (err, result) {
				...
				// if(typeof server.socketOptions.socketTimeoutMS == 'number' && server.socketOptions.socketTimeoutMS > 0)
				//   server._timeoutCalls(server.socketOptions.socketTimeoutMS * 1.1);
			});
		}
	}
